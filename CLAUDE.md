...
2. **Server Side** (`app/api/chat/route.ts`):
   ...
   - Configured with OpenAI provider: `openai("gpt-4o")`
...
- **AI**: OpenAI GPT-4o for intelligent summaries
...
OPENAI_API_KEY=sk-...                               # Required for OpenAI GPT-4o
...
**Notes:**
- The app is configured to use OpenAI's GPT-4o, but can be switched to other providers (Anthropic, Google, etc.) by changing the model in [app/api/chat/route.ts](app/api/chat/route.ts) and [lib/ai/provider.ts](lib/ai/provider.ts)
